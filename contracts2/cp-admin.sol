...
contract Admin {
    int constant tokens = 1000000;

    // unused parameters for now
    int constant blockTokens = 1000000;
    int constant blocksPer = 10;
    ...
    function establish() public returns (bool rc, string reason) {
        if (memberCount > 0) {
    ...
    function proposeNodeBallot(bool join_or_dismissal, bytes name, bytes id, bytes ip, uint port, uint due) internal returns (bool rc, string reason) {
        if (members[msg.sender].tokens <= 0) {
            reason = "Not Allowed";
            return;
        }
        ...
        nodeBallots[id].notBefore = 0;
        nodeBallots[id].notAfter = 0;
    ...
    function nominateNode(bytes name, bytes id, bytes ip, uint port, uint due) public returns (bool rc, string reason) {
        (rc, reason) = proposeNodeBallot(true, name, id, ip, port, due);
    }
    ...
    function dismissNode(bytes id, uint due) public returns (bool rc, string reason) {
        (rc, reason) = proposeNodeBallot(false, "", id, "", 0, due);
    }
    ...
    function settleNodeBallot(bytes id) public returns (bool rc, string reason) {
        ...
        if (acceptTokens >= targetTokens) {
            if (nb.join_or_dismissal) {         // join
                ...
                NodeJoined(nb.name, nb.id, nb.ip, nb.port);
                ...
                NodeDismissed(nb.name, nb.id, nb.ip, nb.port);
                modifiedBlock = block.number;
